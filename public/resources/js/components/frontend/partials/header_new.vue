  ...
  <div>
        ...
        <div class="d-flex justify-content-between">
          ...
          <div class="header-icon d-flex align-items-center">
            <div class="search-container">
              ...
              <input name="search" class="search-input-new" v-model="phoneSearchKey" type="text"
        ...
        <div class="cart-item" v-for="(item, index) in carts" :key="index">
          ...
          <div class="cart-item-details">
            ...
            <div class="cart-item-details-quantity mt-2">
              <p class="mb-0"><span class="item-qnt"> ৳ {{ item.quantity * item.price }} <span
                    class="item-qnt-details">( ৳ {{ item.quantity }} x {{ item.price }} )</span></span> </p>
            </div>
            ...
            <nav class="menu" :class="{ 'active': is_menu_active }">
              ...
              <ul class="menu-main p-0 mb-0 d-lg-flex flex-lg-row justify-content-lg-between d-md-flex flex-md-column">
                <li class="nav-item menu-item-has-children" v-for="(menu, i) in headerMenu" :key=i>
                  <router-link @click.native="subMenuActive(i, menu.url)"
                    :to="menu.url === 'javascript:void(0)' ? '' : menu.url" class="nav-items">
                    {{ menu.label }}
                  </router-link>
    ...
    <div class="mobile-search" :class="is_search_box_active ? 'show' : ''">
      ...
      <div class="mt-5 d-flex justify-content-center position-relative">
        <input type="search" v-model="phoneSearchKey" placeholder="Search" class="form-control search-input"
...
export default {
  ...
  data() {
    return {
      isSticky: false,
      lastScrollY: 0, // Track last scroll position for better smooth effect
      navbarTop: 0,
      mobile_child_id: 0,
  ...
  computed: {
    navbarStyles() {
      return {
        ...
        position: 'fixed', // Always fixed for smoother transition
        top: this.isSticky ? '0' : '-80px', // Start above screen, then slide smoothly
        zIndex: '1000',
        width: '100%',
        padding: '15px',
        backgroundColor: 'rgba(0, 0, 0, 0.9)', // Slightly darker for better visibility
        color: 'white',
        height: '55px',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        boxShadow: this.isSticky ? '0 4px 10px rgba(0, 0, 0, 0.2)' : 'none', // Add shadow when sticky
        transition: 'top 1.0s ease-in-out, box-shadow 0.8s ease-in-out', // Smooth transition effect
  ...
  methods: {
    ...
    handleScroll() {
      ...
      if (navbar) {
        this.isSticky = window.scrollY > navbar.offsetHeight;
      }
    },
