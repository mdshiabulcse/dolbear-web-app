...
<div class="main-sidebar">
    <aside id="sidebar-wrapper">
        <div class="sidebar-brand sidebar-brand-sm">
            <a
                href="{{ route('dashboard') }}">{{ settingHelper('system_short_name') != '' ? settingHelper('system_short_name', app()->getLocale()) : 'Dolbear' }}</a>
        </div>
        <div class="sidebar-brand">
            <a href="{{ route('dashboard') }}">
                <img src="{{ $logo != [] && is_file_exists($logo['image_100x38']) ? static_asset($logo['image_100x38']) : static_asset('images/default/logo.png') }}"
                    alt="Logo"></a>
        </div>
        <ul class="sidebar-menu">
            ...
            {{-- point settings starts--}}
            <li class="nav-item dropdown @yield('point_setting_active')">
                <a href="javaScript:void(0)" class="nav-link has-dropdown" data-toggle="dropdown">
                ...
                <ul class="dropdown-menu">
                    {{-- @if (hasPermission('order_read')) --}}
                        <li class="@yield('point_list_active')"><a class="nav-link"
                                href="{{ route('point.setting') }}">Purchase Point</a></li>
            ...
            {{-- Stores starts--}}
            <li class="nav-item dropdown @yield('store_active')">
                <a href="javaScript:void(0)" class="nav-link has-dropdown" data-toggle="dropdown">
                ...
                <ul class="dropdown-menu">
                    {{-- @if (hasPermission('order_read')) --}}
                    <li class="@yield('all_store_active')"><a class="nav-link"
                        href="{{ route('store.index') }}">All Store</a></li>

                        <li class="@yield('store_create_active')"><a class="nav-link"
                                href="{{ route('store.create') }}">Add New Store</a></li>
            ...
            <li class="@yield('recommended_active')">
                <a class="nav-link" href="{{ route('recommended.index') }}"><i
                        class="bx bx-news"></i><span>Recommended By</span>
                </a>
            </li>

            {{-- Recommended By Ends--}}

            <li class="@yield('flash_message_active')">
                <a class="nav-link" href="{{ route('flash-message.index') }}"><i
                        class="bx bx-chat"></i><span>From Fan</span>
                </a>
            </li>
            ...
            @if (hasPermission('product_read') ||
                    hasPermission('color_read') ||
                    hasPermission('attribute_set_read') ||
                    hasPermission('brand_read') ||
                    hasPermission('category_read') ||
                    hasPermission('attribute_value_read'))
...
{{--                        @endif--}}
                    ...
                    <ul class="dropdown-menu">
                        @if (hasPermission('customer_read'))
                            <li class="@yield('customer_list')">
                                <a class="nav-link" href="{{ route('customers') }}">{{ __('All Customer') }}</a>
                            </li>
                        @endif
            ...
            @if (settingHelper('seller_system') == 1)
                @if (hasPermission('seller_read') ||
                        hasPermission('payout_read') ||
                        hasPermission('seller_commission_read') ||
                        hasPermission('seller_payout_read'))
                ...
                @if (addon_is_activated('seller_subscription') &&
                        (hasPermission('package_read') || hasPermission('subscription_setting_read')))
                    <li class="nav-item dropdown @yield('package_active')">
                        <a href="javascript:void(0)" class="nav-link has-dropdown" data-toggle="dropdown">
                            <i class='bx bx-package {{ config('app.demo_mode') ? 'beep' : '' }}'></i>
            ...
            @if (hasPermission('delivery_hero_read') ||
                    hasPermission('delivery_hero_create') ||
                    hasPermission('delivery_hero_commission_history') ||
                    hasPermission('delivery_hero_deposit_history') ||
                    hasPermission('delivery_hero_collection_history') ||
                    hasPermission('delivery_hero_cancel_request') ||
                    hasPermission('delivery_hero_configuration_read'))
            ...
            @if (hasPermission('admin_product_sale_read') ||
                    hasPermission('seller_product_sale_read') ||
                    hasPermission('product_stock_read') ||
                    hasPermission('product_wishlist_read') ||
                    hasPermission('user_searches_read') ||
                    hasPermission('commission_history_read') ||
                    hasPermission('wallet_recharge_history_read'))
            ...
            @if (hasPermission('campaign_read') ||
                    hasPermission('bulk_sms_read') ||
                    hasPermission('subscriber_read') ||
                    hasPermission('coupon_read') ||
                    hasPermission('campaign_request_read') ||
                    hasPermission('otp_setting_read') ||
                    hasPermission('sms_template_read'))
            ...
            {{-- TODO: all_page_read permission not working --}}
            {{-- @if (hasPermission('all_page_read')) --}}
                <li class="@yield('other_page')"><a class="nav-link" href="{{ route('other.pages') }}"><i
                            class="bx bx-news"></i><span>{{ __('Pages') }}</span></a></li>
            {{-- @endif --}}
            ...
            @if (addon_is_activated('reward'))
                @if (hasPermission('reward_configuration_read') ||
                        hasPermission('reward_setting_read') ||
                        hasPermission('user_reward_read'))
                    ...
                    <ul class="dropdown-menu @yield('shipping')">
                        @if (hasPermission('shipping_configuration_read'))
                            <li class="@yield('shipping-configuration')"><a class="nav-link"
                                    href="{{ route('shipping-configuration') }}">
                                    {{ __('Shipping Configuration') }}</a>
                            </li>
                        @endif
                        ...
                        @if (hasPermission('country_read'))
                            <li class="@yield('available-countries')"><a class="nav-link" href="{{ route('countries') }}">
                                    {{ __('Available Divisions') }}</a>
                            </li>
                        @endif
                        @if (hasPermission('state_read'))
                            <li class="@yield('available-states')"><a class="nav-link" href="{{ route('states') }}">
                                    {{ __('Available District') }}</a>
                            </li>
                        @endif
                        @if (hasPermission('city_read'))
                            <li class="@yield('available-cities')"><a class="nav-link" href="{{ route('cities') }}">
                                    {{ __('Available Thana') }}</a>
                            </li>
                        @endif
                    </ul>
                </li>
            @endif
            @if (
                (hasPermission('theme_option_update') ||
                    hasPermission('header_content_update') ||
                    hasPermission('footer_content_update') ||
                    hasPermission('home_page_update') ||
                    hasPermission('website_seo_update') ||
                    hasPermission('website_popup_update') ||
                    hasPermission('custom_css_update') ||
                    hasPermission('custom_js_update') ||
                    hasPermission('gdpr_update') ||
                    hasPermission('slider_read') ||
                    hasPermission('service_read') ||
                    hasPermission('all_page_read') ||
                    hasPermission('login_singup_read')) &&
                    !config('app.mobile_mode'))
                <li
                    class="nav-item dropdown @yield('store_front_active') @yield('header_content') @yield('footer_content') @yield('slider_active') @yield('service_active')  @yield('banners')">
                    <a href="javaScript:void(0)" class="nav-link has-dropdown" data-toggle="dropdown"><i
                    ...
                    <ul class="dropdown-menu @yield('store-front')">
                        ...
                        @if (hasPermission('header_content_update'))
                            <li class="@yield('header_content')"><a class="nav-link"
                                    href="{{ route('menu') }}">{{ __('Header Content') }}</a>
                            </li>
                        @endif
                        ...
                        @if (hasPermission('slider_read'))
                            <li class="@yield('slider_active')">
                                <a class="nav-link"
                                    href="{{ route('sliders.index') }}"><span>{{ __('Slider') }}</span>
                                </a>
                            </li>
                        @endif
                        ...
                        @endif
                        <li class="@yield('home_page_video')">
                            <a class="nav-link"
                                href="{{ route('sliders.index') }}"><span>{{ __('Home Page Video') }}</span>
                            </a>
                        </li>
            ...
            @if (hasPermission('language_read') ||
                    hasPermission('language_create') ||
                    hasPermission('language_update') ||
                    hasPermission('general_setting_update') ||
                    hasPermission('preference_setting_update') ||
                    hasPermission('email_setting_update') ||
                    hasPermission('currency_setting_update') ||
                    hasPermission('vat_tax_setting_update') ||
                    hasPermission('storage_setting_update') ||
                    hasPermission('cache_update') ||
                    hasPermission('miscellaneous_setting_update') ||
                    hasPermission('admin_panel_setting_update') ||
                ...
                <li class="nav-item dropdown @yield('setup')">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        @endif
                        @if (hasPermission('google_service_update'))
                            <li class="@yield('google_recaptcha_active')"><a href="{{ route('settings.google.recaptcha') }}"
                        ...
                        @endif
                        @if (hasPermission('pusher_notification_update'))
                            <li class="@yield('firebase_update')"><a href="{{ route('settings.firebase') }}"
            ...
            @if (hasPermission('staff_read') || hasPermission('role_read'))
                <li class="@yield('staffs')">
                    <a href="{{ route('staffs') }}" class="nav-link"><i class="bx bx-group"></i>
                        <span>{{ __('Manage Staffs') }}</span></a>
                </li>
            @endif

            @if (hasPermission('android_setting_update') ||
                    hasPermission('ios_setting_update') ||
                    hasPermission('app_config_update') ||
                    hasPermission('ads_config_update') ||
                    hasPermission('api_setting_update') ||
                    hasPermission('api_key_read_all') ||
                    hasPermission('api_key_read') ||
                    hasPermission('api_key_update') ||
                    hasPermission('api_key_delete') ||
                    hasPermission('all_page_read'))
            ...
            @if (hasPermission('addon_read'))
                @if (settingHelper('current_version') != '1.0.0')
                    <li class="nav-item dropdown @yield('addon_utility')">
                        <a href="javaScript:void(0)" class="nav-link has-dropdown" data-toggle="dropdown"><i
                                class="bx bx-extension" aria-hidden="true"></i><span>{{ __('Addons') }}</span></a>
                        <ul class="dropdown-menu">
