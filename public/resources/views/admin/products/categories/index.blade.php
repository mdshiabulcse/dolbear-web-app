...
@section('main-content')
    <section class="section">
        <div class="section-body ">
            ...
            <div class="row">
                <div
                    class="{{ hasPermission('category_create') ? 'col-sm-xs-12 col-md-8' : 'col-sm-xs-12 col-md-8 middle' }}">
                    <div class="card">
                        <form action="">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($categories as $key => $category)
                                            <tr id="row_{{ $category->id }}" class="table-data-row">
                                                ...
                                                <td>
                                                    <div class="ml-1"><span
                                                            class="{{ $category->icon != null || $category->icon != '' ? $category->icon : 'mdi mdi-view-dashboard-outline' }}"></span>
                                                ...
                                                <td>
                                                    <label
                                                        ...
                                                        <input type="checkbox"
                                                            value="category-featured-change/{{ $category->id }}"
                                                            {{ $category->is_featured == 1 ? 'checked' : '' }}
                                                            {{ hasPermission('category_update') ? '' : 'disabled' }}
                                                ...
                                                <td>
                                                    ...
                                                    @if (hasPermission('category_delete'))
                                                        <a href="javascript:void(0)"
                                                            ...
                                                            title="" data-original-title="{{ __('Delete') }}"><i
                                ...
                                <form method="POST" action="{{ route('store.categories') }}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="title" id="title"
                                            value="{{ old('title') }}" placeholder="{{ __('Title') }}" tabindex="1">
                                        @if ($errors->has('title'))
                                        ...
                                        <select class="form-control selectric lang" name="category">
                                            ...
                                            @foreach ($all_categories as $key => $category)
                                                <option value="{{ $category->id }}"
                                                    ...
                                                    {{ $category->getTranslation('title', \App::getLocale()) }}</option>
                                                @foreach ($category->childCategories as $childCategory)
                                                    @include('admin.products.categories.child-categories', [
                                    ...
                                    <div class="form-group" style="display: none;">
                                        ...
                                        <input id="commission" type="number" class="form-control" step="any"
                                            ...
                                            placeholder="{{ __('Commission Rate') }} {{ __('(%)') }}"
                                            tabindex="1">
                                        ...
                                        <div class="form-group">
                                            <div class="input-group gallery-modal" id="btnSubmit" data-for="image"
                                                ...
                                                <span class="form-control"><span
                                                        class="counter">{{ old('logo') != '' ? substr_count(old('logo'), ',') + 1 : 0 }}</span>
                                                    {{ __('file chosen') }}</span>
                                        ...
                                        <div class="form-group">
                                            <div class="input-group gallery-modal" id="btnSubmit" data-for="image"
                                                ...
                                                <input type="hidden" name="banner"
                                                    ...
                                                    class="image-selected">
                                                <span class="form-control"><span
                                                        class="counter">{{ old('banner') != '' ? substr_count(old('banner'), ',') + 1 : 0 }}</span>
                                                    {{ __('file chosen') }}</span>
                                                <div class="input-group-prepend">
                                                    <div class="input-group-text">
                                                        {{ __('Choose File') }}
    ...
    <script>
        (function() {
            ...
            fetch('{{ static_asset('admin/js/meta.json') }}').then((response) => {
                ...
                $('input[type="text"].use-material-icon-picker').each(function() {
                    ...
                    $search.on('keyup', function() {
                        ...
                        $icons.find('.mdi-custom-icon:contains(' + search + ')').css('display',
                            'inline-block');
                    });
                    ...
                    function iconAppend(icons, limit) {
                        icons.forEach(function(icon, index) {
                            if (index < limit) {
                                ...
                                var $icon = $('<div class="mdi-custom-icon" title="' + icon
                                    .name + '"><i class="' + class_name + '"></i><p>' + icon
                                    .name + '</p></div>');
            ...
            $(document).on('mouseup', function(e) {
                ...
                if ($picker.length && !$picker.is(e.target) && !$(e.target).hasClass(
                    'use-material-icon-picker') && $picker.has(e.target).length === 0) {
                    $picker.fadeOut(200);
                }
            });
