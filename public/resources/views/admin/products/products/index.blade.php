                                ...
                                <form class="form-inline">
                                    {{-- @if (settingHelper('seller_system') == 1)
                                        <div class="form-group">
                                            <select class="seller-by-ajax form-control select2" name="sl"
                                                id="seller_id" aria-hidden="true">
                                                @if (isset($sl))
                                                    <option selected value="{{ $selected_seller->id }}">
                                                        {{ $selected_seller->shop_name }} </option>
                                                @endif
                                            </select>
                                        </div>
                                    @endif --}}
                                    <div class="form-group">
                                        ...
                                        @endphp
                                        <select class="filter-categories-by-ajax form-control select2" name="c"
                                            id="c">
                                            <option value="">{{ __('All Category') }}</option>
                                            @if (isset($c))
                                                @if ($selected_category != null)
                                                    <option selected value="{{ @$c }}">
                                                        {{ $position . @$selected_category->getTranslation('title', App::getLocale()) }}
                                                    </option>
                                                @endif
                                            @endif
                                        </select>
                                    ...
                                    <div class="form-group">
                                        <select class="form-control selectric" name="s">
                                            ...
                                            <option value="sale_high" {{ $s == 'sale_high' ? 'selected' : '' }}>
                                                {{ __('Sale(High > Low)') }}</option>
                                            <option value="sale_low" {{ $s == 'sale_low' ? 'selected' : '' }}>
                                                {{ __('Sale(Low > High)') }}</option>
...
{{--                                            <th>{{ __("Today's Deal") }}</th>--}}
                                            <th>{{ __('Featured') }}</th>
                                            <th>{{ __('New Arrived') }}</th>
                                            <th>{{ __('Best Seller') }}</th>
                                            <th>{{ __('Bundle Deals') }}</th>
                                            @if (hasPermission('product_update') ||
                                                    hasPermission('product_delete') ||
                                                    hasPermission('product_restore') ||
                                                    hasPermission('product_clone'))
                                                <th>{{ __('Option') }}</th>
                                            @endif
                                        </tr>
                                    </thead>
                                        ...
                                        @foreach ($products as $key => $product)
                                            <tr id="row_{{ $product->id }}">
                                                ...
                                                <td width="300">
                                                    <div class="d-flex">
                                                        ...
                                                        <div class="ml-1">
                                                            @if (isAppMode())
                                                                <a
                                                                    href="#">{{ $product->getTranslation('name', \App::getLocale()) }}</a>
                                                            @else
                                                ...
                                                {{-- @if (settingHelper('seller_system') == 1)
                                                    <td>
                                                        @if ($product->user_id != 1)
                                                            ...
                                                            @endif
                                                        @else
                                                            <div class="badge badge-warning">{{ __('Admin Product') }}
                                                            </div>
                                                        @endif
                                                    </td>
                                                @endif --}}
                                                <td>
                                                    ...
                                                    <span>{{ __('Rating') . ': ' . $product->rating }}</span> --}}
                                                    <span>{{ __('Current Stock') . ': ' . $product->stock->sum('current_stock') }}</span><br>
                                                ...
                                                <td>
                                                    @if ($product->is_approved == 0)
                                                        <div class="d-flex">
                                                            <div class="ml-1 badge badge-pill badge-warning">
                                                                {{ __('Pending') }}</div>
                                                        </div>
                                                    @else
                                                        <label
                                                            class="custom-switch mt-2 {{ hasPermission('product_update') && $product->status != 'trash' ? '' : 'cursor-not-allowed' }}">
                                                            <input type="checkbox"
...
{{--                                                </td>--}}
                                                ...
                                                <td>
                                                    <label
                                                            class="custom-switch mt-2 {{ hasPermission('product_update') && $product->status != 'trash' ? '' : 'cursor-not-allowed' }}">
                                                        <input type="checkbox"
                                                               @if (hasPermission('product_update') && $product->status != 'trash') value="product-status-change/{{ $product->id }}/is_new_arrived" @endif
                                                               {{ $product->is_new_arrived == 1 ? 'checked' : '' }}
                                                               name="custom-switch-checkbox"
                                                               {{ hasPermission('product_update') && $product->status != 'trash' ? '' : 'disabled' }}
                                                               class="{{ hasPermission('product_update') && $product->status != 'trash' ? 'product-status-change' : '' }} custom-switch-input">
                                                        <span class="custom-switch-indicator"></span>
                                                ...
                                                <td>
                                                    <label
                                                            class="custom-switch mt-2 {{ hasPermission('product_update') && $product->status != 'trash' ? '' : 'cursor-not-allowed' }}">
                                                        <input type="checkbox"
                                                               @if (hasPermission('product_update') && $product->status != 'trash') value="product-status-change/{{ $product->id }}/is_best_seller" @endif
                                                ...
                                                <td>
                                                    <label
                                                            class="custom-switch mt-2 {{ hasPermission('product_update') && $product->status != 'trash' ? '' : 'cursor-not-allowed' }}">
                                                        <input type="checkbox"
                                                               @if (hasPermission('product_update') && $product->status != 'trash') value="product-status-change/{{ $product->id }}/is_bundle_deals" @endif
                                                               {{ $product->is_bundle_deals == 1 ? 'checked' : '' }}
                                                               name="custom-switch-checkbox"
                                                               {{ hasPermission('product_update') && $product->status != 'trash' ? '' : 'disabled' }}
                                                               class="{{ hasPermission('product_update') && $product->status != 'trash' ? 'product-status-change' : '' }} custom-switch-input">
                                                        <span class="custom-switch-indicator"></span>
                                                    ...
                                                    @if ($product->status != 'trash')
                                                        ...
                                                        {{-- @if (hasPermission('product_clone'))
                                                            <a href="{{ route('product.clone', $product->id) }}"
                                                                ...
                                                                data-toggle="tooltip" title=""
                                                                data-original-title="{{ __('Clone') }}"><i
                                                                    class='bx bx-copy'></i>
                                                            </a>
                                                        @endif --}}
                                                        ...
                                                        <div class="dropdown-menu">
                                                            <a href="{{ isAppMode() ? '#' : route('product-details', $product->slug) }}"
                                                            ...
                                                            @if (hasPermission('product_update'))
                                                                @if ($product->is_approved == 1)
                                                                    <a href="{{ route('product.status.change', ['status' => 'pending', 'id' => $product->id]) }}"
                                                                        class="dropdown-item has-icon"><i
                                                                            class='bx bx-block'></i>{{ __('Pending') }}
                                                                    </a>
                                                                @else
                                                                ...
                                                                @endif
                                                                @if (addon_is_activated('wholesale') && !$product->is_wholesale)
