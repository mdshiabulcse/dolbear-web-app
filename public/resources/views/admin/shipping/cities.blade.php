    ...
    <section class="section city_section">
        <div class="section-body">
            <div class="d-flex justify-content-between">
                <div class="d-block">
                    <h2 class="section-title">{{ __('Thana') }}</h2>
                    <p class="section-lead">
                        {{ __('You have total') . ' ' . $cities->total() . ' ' . __('City') }}
                    </p>
                </div>
                @if(hasPermission('city_import_create'))
            ...
            <div class="row">
                <div class="{{ hasPermission('city_create') ? 'col-8 col-md-8 col-lg-8' : 'col-8 col-md-8 col-lg-8 middle' }}">
                    <div class="card">
                            ...
                            <div class="card-header-form">
                                <form class="form-inline" id="sorting">
                                    <div class="form-group">
                                        <select class="state-by-ajax form-control sorting select2" name="a" id="a"
                                                required>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-md city_table_font">
                                    ...
                                    <tr>
                                        ...
                                        {{-- <th>{{ __('Division') }}</th> --}}
                                        <th>{{ __('COD Cost') }}</th>
                                        <th>{{ __('Express Delivery Cost') }}</th>
                                        <th>{{ __('Status') }}</th>
                                    ...
                                    @foreach($cities as $key => $value)
                                        <tr id="{{ $key }}">
                                            ...
                                            <td> {{ get_price($value->express_delivery_cost) }} </td>
                                            <td>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body card-body-paddding">
                                <form method="POST" action="{{ route('city.store') }}">
                                    @csrf
                                    <div class="form-group">
                                        <label for="state_id">{{ __('District') }}</label>
                                        <select class="state-by-ajax form-control select2" name="state_id" id="state_id"
                                                required>
                                            <option value="">{{ __('Select District') }}</option>
                                        </select>
                                    ...
                                    <div class="form-group">
                                        <label for="express_delivery_cost">{{ __('Express Delivery Cost') }}</label>
                                        <input type="number" step="any" name="express_delivery_cost" id="express_delivery_cost"
                                               placeholder="{{ __('Express delivery cost') }}" value="{{ old('express_delivery_cost') }}"
                                               class="form-control" required>
                                        @if ($errors->has('express_delivery_cost'))
                                            <div class="invalid-feedback">
                                                <p>{{ $errors->first('express_delivery_cost') }}</p>
                                            </div>
                                        @endif
                                    </div>
