                            ...
                            <form action="{{ route('state.update') }}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="modal-body modal-padding-bottom">
                                    ...
                                    <div class="form-group" >
                                        <label for="country_id_c">{{ __('Division') }}</label>
                                        <select class="form-control select2" name="division_id" id ="country_id_c" required>
                                            <option value="">{{ __('Select Division') }}</option>
                                            @foreach($divisions as $key => $country)
                                                <option {{ $country->id == $state->division_id ? "selected" : "" }} value="{{ $country->id }}">{{ $country->name }}</option>
                                            @endforeach
                                        </select>
