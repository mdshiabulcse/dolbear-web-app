    ...
    <section class="section">
        <div class="section-body">
            <div class="d-flex justify-content-between">
                <div class="d-block">
                    <h2 class="section-title">{{ __('District') }}</h2>
                    <p class="section-lead">
                        {{ __('You have total') . ' ' . $states->total() . ' ' . __('District') }}
                    </p>
                </div>
                @if(hasPermission('state_import_create'))
                <div class="mt-4">
                                ...
                                <form class="form-inline" id="sorting">
                                    {{-- <div class="form-group">
                                        <select class="form-control select2 sorting" name="a">
                                            <option value="">{{ __('Filter By Divisions') }}</option>
                                            @foreach($divisions as $key => $country)
                                ...
                                <table class="table table-striped table-md">
                                    ...
                                    @foreach($states as $key => $value)
                                        <tr id="{{ $states->firstItem() + $key }}">
                                            ...
                                            <td> {{ $value->name }} </td>
                                            <td> {{ $value->division->name ?? null}} </td>
                    ...
                    <div class="card" >
                            ...
                            <div class="card-body card-body-paddding">
                                <form method="POST" action="{{ route('state.store') }}" enctype="multipart/form-data">
                                    @csrf
                                    <div class="form-group" >
                                        <label for="country_id">{{ __('Division') }}</label>
                                        <select class="form-control select2" name="division_id" id ="country_id" required>
