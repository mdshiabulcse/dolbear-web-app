...
jQuery(function ($) {
  ...
  $(document).ready(function () {
    $(".notification-status").on("click", function (e) {
      ...
      if (notification.status == "seen") {
        ...
        custom_url =
          url + "/" + "notification/status-change/" + notification.id;
      }
      $.ajax({
        type: "GET",
        dataType: "html",
  ...
  $(document).ready(function () {
    ...
    $(document).on("change", ".status-change", function (e) {
      ...
      var url = $("#url").val() + "/" + user_type + "/" + value[0];
    ...
    $(document).on("click", ".subscription-status-change", function (e) {
      e.preventDefault();
      var token = $('meta[name="csrf-token"]').attr("content");
      let selector = this;
      var url = $("#url").val();
      ...
      let msg_1 = $(".product_alert_danger").val();
      let msg_2 = $(".product_alert_success").val();

      if (type == "package") {
        msg_1 = $(".package_danger").val();
        msg_2 = $(".package_success").val();
      }

      Swal.fire({
        title: $(".sure").val(),
        text: status == 1 ? msg_2 : msg_1,
        icon: "warning",
  ...
  $(document).on("click", "#banner_delete", function () {
    ...
    $("#img_payment_method_banner").attr(
      "src",
      "" + url + "public/images/default/default-image-72x72.png"
    );
    selector.hide();
    $(".delete-image").remove();
    $.ajax({
  ...
  $(".onChangeFormSubmitStore").on("change", function () {
    $("#onChangeFormSubmitStore").submit();
  });
  ...
  $(document).ready(function () {
    ...
    $(document).on("click", "#add-item", function () {
      ...
      if (content_area === "content-area") {
        ...
        selector
          .find(".wholesale_min_qty")
          .attr("data-id", wholesale_product_id);
        selector
          .find(".wholesale_max_qty")
          .attr("data-id", wholesale_product_id);
  ...
  $(document).on(
    "keypress input paste focusin focusout",
    "#short_description",
    function (e) {
  ...
  $(document).on("click", ".remove-menu-row", function (e) {
    ...
    var type = $(this).attr("data-type");
    if (type === "banner-image") {
      let number_of_banner_images = $(this)
        .closest(".menu-item")
        .find(".banner-item").length;
  ...
  $(document).ready(function () {
    ...
    $(document).on("click", "#download_update", function () {
      ...
      $.ajax({
        ...
        error: function (data) {
          $(".alert_div").addClass("alert-danger").removeClass("d-none");
...
function addButton(content, button) {
  let number_of_banner_images = content
    .closest(".menu-item")
    .find(".banner-item")
