...
export default {
  ...
  mounted() {
    ...
    script.src = this.getUrl("frontend/js/k.js");
    let amount =
      this.type == "wallet"
        ? this.round(
            (this.amount / this.activeCurrency.exchange_rate) *
              this.xof.exchange_rate
          )
        : this.round(this.amount * this.xof.exchange_rate);
    ...
    let url =
      this.type == "wallet"
        ? this.getUrl(
            "user/recharge-wallet?amount=" +
              this.amount +
              "&type=wallet&payment_type=kkiapay"
          )
        : this.getUrl(
            "user/complete-order?payment_type=kkiapay&code=" +
              this.code +
              "&trx_id=" +
              this.trx_id
          );
      ...
      axios
        ...
        .then((response) => {
          if (response.data.error) {
            toastr.error(
              this.lang.something_went_wrong,
              this.lang.Error + " !!"
            );
          } else {
