                        ...
                        <form method="POST" action="{{ route('coupon.update') }}" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <div class="custom-file">
                                    <select class="form-control selectric" disabled id="couponType" >
                                        <option
                                            {{ $coupon_language->coupon->type == 'product_base' ? "selected" : "" }} value="product_base">{{ __('Product Base') }}</option>
                                        <option
                                            {{ $coupon_language->coupon->type == 'invoice_base' ? "selected" : "" }} value="invoice_base">{{ __('Invoice Base') }}</option>
                                    </select>
                                </div>
                                @if ($errors->has('type'))
                                    <div class="invalid-feedback">
                                ...
                                <div class="form-group">
                                    <div class="input-group gallery-modal" id="btnSubmit" data-for="image"
                                        ...
                                        <input type="hidden" name="banner"
                                               value="{{ $coupon_language->coupon->banner_id }}" class="image-selected">
                                        <span class="form-control"><span
                                                class="counter">{{ $coupon_language->coupon->banner_id ? 1 : 0 }}</span> {{ __('file chosen') }}</span>
                                    ...
                                    <div class="selected-media-box">
                                        <div class="mt-4 gallery gallery-md d-flex">
                                            <div class="selected-media mt-0 m-2"
                                                 data-id="{{ $coupon_language->coupon->banner_id }}">
                                                @if ($coupon_language->coupon->banner != [] && @is_file_exists(@$coupon_language->coupon->banner['image_72x72'], $coupon_language->coupon->banner['storage']))
                                                    <img
                                                        src="{{ get_media($coupon_language->coupon->banner['image_72x72'], $coupon_language->coupon->banner['storage']) }}"
                                                        alt="{{ $coupon_language->title }}"
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="code" id="code"
                                       value="{{ old('code') ? old('code') : $coupon_language->coupon->code }}"
                                       class="form-control" required>
                                <input type="hidden" name="id" value="{{ $coupon_language->coupon->id }}" >
                                <input type="hidden" value="{{ old('r') ? old('r') : (@$r ? $r : url()->previous() )}}"
                                       name="r">
                                <input type="hidden" name="type" value="{{ $coupon_language->coupon->type }}" >
                                @if ($errors->has('code'))
                                    <div class="invalid-feedback">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" value="{{ old('date') ? old('date') : $date }}" name="date"
                                           id="date" class="form-control daterange-cus">
                                </div>
                                @if ($errors->has('date'))
                                    <div class="invalid-feedback">
                                ...
                                <tr>
                                    <td width="30%" class="pl-0 pr-0">
                                        ...
                                        <select class="form-control selectric" name="discount_type"
                                                data-title="discount_type" id="discount_type">
                                            <option
                                                {{ $coupon_language->coupon->discount_type == 'flat' ? "selected" : "" }} value="flat">{{ __('Flat') }}</option>
                                            <option
                                                {{ $coupon_language->coupon->discount_type == 'percent' ? "selected" : "" }} value="percent">{{ __('Percent') }}</option>
                                        </select>
                                    ...
                                    <td width="70%" class="pl-0 pr-0">
                                        ...
                                        <input type="number" step="any" name="discount" id="discount"
                                               value="{{ old('discount',$coupon_language->coupon->discount_type == 'percent' ? $coupon_language->coupon->discount : priceFormatUpdate($coupon_language->coupon->discount,settingHelper('default_currency'),$type="*")) }}"
                                               class="form-control" >
                            ...
                            @if($coupon_language->coupon->type != "invoice_base")
                                <div class="form-group" id="div_product_base">
                                    ...
                                    <select class="product-by-ajax form-control select2" id="product_id"
                                            multiple="multiple" name="product_id[]" aria-hidden="true">
                                        @if($products)
                                            @foreach($products as $key => $product)
                                                <option
                                                    {{ old('product_id') ? (in_array($product->id, old('product_id')) ? "selected" : "" ) : (in_array($product->id, $coupon_language->coupon->product_id) ? "selected" : "")}} value=" {{ $product->id }}">{{ $product->getTranslation('name', \App::getLocale()) }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <div class="" id="div_invoice_base">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="minimum_shopping" id="minimum_shopping"
                                               value="{{ old('minimum_shopping') ? old('minimum_shopping') : priceFormatUpdate( $coupon_language->coupon->minimum_shopping,settingHelper('default_currency'),$type="*") }}"
                                               class="form-control">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" step="any" name="maximum_discount" id="maximum_discount"
                                               value="{{ old('maximum_discount') ? old('maximum_discount') : priceFormatUpdate( $coupon_language->coupon->maximum_discount,settingHelper('default_currency'),$type="*")}}"
                                               class="form-control">
