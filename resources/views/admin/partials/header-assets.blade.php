<head>
    ...
    <title>@yield('title') |
        {{ settingHelper('admin_panel_title') != '' ? settingHelper('admin_panel_title') : __('Dolbear') }}</title>
    @php
        $logo = settingHelper('og_image');
    @endphp

    @if ($logo != [] && $logo['original_image'] != '')
        <meta property="og:image" content="{{ get_media(settingHelper('og_image')['original_image']) }}" />
    @else
        <meta property="og:image" content="{{ static_asset('default-image/default-730x400.png') }}"
            alt="{!! settingHelper('meta_title') !!}" />
    @endif
    <!-- General CSS Files -->
    ...
    <link rel="stylesheet"
        href="{{ static_asset('admin/css/style.css') }}?version={{ settingHelper('current_version') }}">
    ...
    @endif
    <link rel="stylesheet"
        href="{{ static_asset('admin/css/custom.css') }}?version={{ settingHelper('current_version') }}">
