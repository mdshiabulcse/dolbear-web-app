                    ...
                    <div class="card-header-form float-right form-inline">
                        ...
                        <div class="form-group">
                            <select
                                ...
                                <option
                                    {{ $delivery_hero->deliveryHero->id == $order->delivery_hero_id ? "selected" : "" }} value="{{ $delivery_hero->deliveryHero->id }}">{{ $delivery_hero->first_name. ' '.$delivery_hero->last_name  }}
                                </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        @if(($order->delivery_status == "confirm" && ($order->delivery_method !== '' || $order->delivery_method !== \App\Constants\DeliveryMethodConstant::PICK_FROM_STORE)) || $order->pathao_delivery_id)
                            <a href="{{ route('admin.delivery.pathao', ['orderId' => $order->id]) }}"
                               class="btn btn-danger btn-circle mr-1" data-url=""
                               data-toggle="tooltip" title=""
                               data-original-title="Pathao Delivery">
                                <i class='bx bx-package'></i> Pathao
                            </a>
                        @endif
                        ...
                        <form action="{{ route('order.payment.status.change') }}" method="POST"
                            ...
                            <div class="form-group">
                                <select
                                    class="form-control selectric {{ $order->delivery_status == "canceled"  ? 'cursor-not-allowed' : '' }}"
                                    id="payment_status"
                                    name="payment_status"
                                    ...
                                    {{-- @if($order->payment_status == "refunded_to_wallet" || $order->payment_type == 'cash_on_delivery' || $order->payment_type == 'pickup_from_shop' || $order->pickupHub)
                                                <option
                        ...
                        <form action="{{ route('order.delivery.status.change') }}" method="POST" class=""
                            ...
                            <div style="display: flex; justify-content: space-between;">
                                <div class="form-group" style="width: 45%;">
                                    <select class="form-control selectric onChangeFormSubmit {{ $order->delivery_status == "delivered" || $order->delivery_status == "canceled" ? 'cursor-not-allowed' : '' }}"
                                        name="delivery_status" {{ $order->delivery_status == "delivered" || $order->delivery_status == "canceled" ? 'disabled' : '' }}>
                                        <option
                                            {{ $order->delivery_status == "pending" ? "selected" : "" }} value="pending">{{ __('Pending') }}</option>

                                        <option
                                                {{ $order->delivery_status == "no_ans_1" ? "selected" : "" }} value="no_ans_1">No Ans 1</option>

                                        <option
                                                {{ $order->delivery_status == "no_ans_2" ? "selected" : "" }} value="no_ans_2">No Ans 2</option>

                                        <option
                                                {{ $order->delivery_status == "no_ans_3" ? "selected" : "" }} value="no_ans_3">No Ans 3</option>
                                ...
                                <div class="form-group" style="width: 45%;">
                                    <select @if($order->store_id || (!$order->store_id && $order->delivery_status !== "pending")) disabled @endif class="form-control selectric" id="store" name="store">
                                        @foreach ($stores as $store)

                                        <option value="{{ $store->id }}"
                                            @if($order->store_id == $store->id) selected @endif>
                                            {{ \Illuminate\Support\Str::limit($store->name, 25, '...') }}
                                        </option>
                        ...
                        <div class="row">
                            <div class="col-md-6 col-12 col-sm-6">
                                <address>
                                    ...
                                    <br>
                                    {{ @$order->billing_address['thana'] }} {{ @$order->billing_address['thana'] ? ',' : '' }}
                                    {{ @$order->billing_address['district'] }} {{ @$order->billing_address['district'] ? ',' : '' }}
                                    {{ @$order->billing_address['division'] }}
                                </address>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover table-md">
                                <tr>
                                    ...
                                    <th class="text-center" width="20%">{{__('Unit Price')}}</th>
                                    ...
                                    <th class="text-right" width="15%">{{__('Totals')}}</th>
                                </tr>
                                @foreach ($order->orderDetails as $key => $orderDetail)
                                @php
...
{{-- <script>
    ...
    document.getElementById('store').addEventListener('change', function() {
        // Submit the form when the select item value is changed
        document.getElementById('onChangeFormSubmit3').submit();
    });
</script> --}}