...
{{--    @endif--}}
    ...
    {{-- ============================================= --}}
    {{-- GOOGLE ANALYTICS 4 (GA4) - Manual Setup     --}}
    {{-- ============================================= --}}
    <!-- Google tag (gtag.js) -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-KL42482RJJ"></script>
    <script>
        ...
        gtag('config', 'G-KL42482RJJ');
    </script>

    {{-- ============================================= --}}
    {{-- FACEBOOK PIXEL - Manual Setup with Advanced Matching --}}
    {{-- ============================================= --}}
    ...
    <script>
        ...
        if (!window.fbPixelInitialized || typeof window.fbq === 'undefined') {
            ...
            // Advanced Matching - Prepare user data object
            var fbAdvancedMatchingData = {};
            var isLoggedIn = false;

            // Check if user is authenticated using Sentinel
            @if(Sentinel::check() && Sentinel::getUser())
                @php $currentUser = Sentinel::getUser(); @endphp
                fbAdvancedMatchingData = {
                    em: '{{ $currentUser->email ?? "" }}',
                    fn: '{{ $currentUser->first_name ?? "" }}',
                    ln: '{{ $currentUser->last_name ?? "" }}',
                    ph: '{{ $currentUser->phone ?? "" }}',
                    ge: '{{ $currentUser->gender ?? "" }}',
                    db: '{{ $currentUser->date_of_birth ?? "" }}',
    ...
    <noscript>
        <img height="1" width="1" style="display:none"
             src="https://www.facebook.com/tr?id=756455577290976&ev=PageView&noscript=1"/>
    </noscript>
    <!-- End Facebook Pixel Code with Advanced Matching -->
    ...
    <script>
        ...
        window.storeFacebookPixelUser = function(userData) {
            ...
            // Store user data globally for future events
            ...
            console.log('[Facebook Pixel] User data stored:', userData.email);
        };

        // Helper to check if user is logged in for Facebook Pixel
        window.isFbPixelLoggedIn = function() {
            return window.fbPixelData && window.fbPixelData.isLoggedIn;
        };
    </script>
...
<script src="https://t.contentsquare.net/uxa/f81260905e668.js"></script>
<!-- End Hotjar Tag Manager (noscript) -->
...
<!-- End clarity Tag Manager (noscript) -->
