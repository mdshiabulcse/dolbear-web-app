...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    {
        Log::info('SSLCOMMERZ Order Validation: Entry point called', [
            'trx_id' => $trx_id,
            'amount' => $amount,
            'currency' => $currency,
            'has_post_data' => !empty($post_data),
            'is_array' => is_array($post_data)
        ]);

        if ($post_data == '' && $trx_id == '' && !is_array($post_data)) {
            Log::error('SSLCOMMERZ Order Validation: Invalid parameters provided', [
                'trx_id' => $trx_id,
                'amount' => $amount
            ]);
        ...
        if ($validation) {
            Log::info('SSLCOMMERZ Order Validation: Validation passed', [
                'trx_id' => $trx_id
            ]);
            return true;
        } else {
            Log::error('SSLCOMMERZ Order Validation: Validation failed', [
                'trx_id' => $trx_id,
                'error' => $this->error
            ]);
            return false;
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if ($merchant_trans_id != "" && $merchant_trans_amount != 0) {
            ...
            Log::info('SSLCOMMERZ Validation: Starting transaction validation', [
                'tran_id' => $merchant_trans_id,
                'amount' => $merchant_trans_amount,
                'currency' => $merchant_trans_currency
            ]);
            ...
            Log::info('SSLCOMMERZ Validation: Verifying hash signature');

            if ($this->SSLCOMMERZ_hash_verify($post_data, $this->getStorePassword())) {

                Log::info('SSLCOMMERZ Validation: Hash verification successful', [
                    'tran_id' => $merchant_trans_id
                ]);
                ...
                Log::info('SSLCOMMERZ Validation: Calling API', [
                ...
                Log::info('SSLCOMMERZ Validation: API Response received', [
                    'http_code' => $code,
                ...
                if ($code == 200 && !(curl_errno($handle))) {
                    ...
                    $tran_date = $result->tran_date ?? null;
                    $tran_id = $result->tran_id ?? null;
                    $val_id = $result->val_id ?? null;
                    $amount = $result->amount ?? null;
                    $store_amount = $result->store_amount ?? null;
                    $bank_tran_id = $result->bank_tran_id ?? null;
                    $card_type = $result->card_type ?? null;
                    $currency_type = $result->currency_type ?? null;
                    $currency_amount = $result->currency_amount ?? null;

                    # ISSUER INFO
                    $card_no = $result->card_no ?? null;
                    $card_issuer = $result->card_issuer ?? null;
                    $card_brand = $result->card_brand ?? null;
                    $card_issuer_country = $result->card_issuer_country ?? null;
                    $card_issuer_country_code = $result->card_issuer_country_code ?? null;

                    # API AUTHENTICATION
                    $APIConnect = $result->APIConnect ?? null;
                    $validated_on = $result->validated_on ?? null;
                    $gw_version = $result->gw_version ?? null;

                    Log::info('SSLCOMMERZ Validation: Transaction details', [
                        'status' => $status,
                        'tran_id' => $tran_id,
                        'amount' => $amount,
                    ...
                    if ($status == "VALID" || $status == "VALIDATED") {
                        Log::info('SSLCOMMERZ Validation: Transaction is VALID/VALIDATED, verifying data integrity', [
                            'merchant_tran_id' => $merchant_trans_id,
                            'api_tran_id' => $tran_id,
                            'merchant_amount' => $merchant_trans_amount,
                            'api_amount' => $amount,
                            'merchant_currency' => $merchant_trans_currency,
                            'api_currency' => $currency_type
                        ]);
                        ...
                        if ($merchant_trans_currency == "BDT") {
                            if (trim($merchant_trans_id) == trim($tran_id) && (abs($merchant_trans_amount - $amount) < 1) && trim($merchant_trans_currency) == trim('BDT')) {
                                Log::info('SSLCOMMERZ Validation: Transaction validated successfully', [
                                    'tran_id' => $tran_id,
                                    'amount' => $amount
                                ]);
                                return true;
                                ...
                                # DATA TEMPERED
                                Log::error('SSLCOMMERZ Validation: Data tampering detected', [
                                    'merchant_tran_id' => $merchant_trans_id,
                                    'api_tran_id' => $tran_id,
                                    'merchant_amount' => $merchant_trans_amount,
                                    'api_amount' => $amount,
                                    'id_match' => trim($merchant_trans_id) == trim($tran_id),
                                    'amount_match' => abs($merchant_trans_amount - $amount) < 1
                                ]);
                            ...
                            if (trim($merchant_trans_id) == trim($tran_id) && (abs($merchant_trans_amount - $currency_amount) < 1) && trim($merchant_trans_currency) == trim($currency_type)) {
                                Log::info('SSLCOMMERZ Validation: Transaction validated successfully (non-BDT)', [
                                    'tran_id' => $tran_id,
                                ...
                                # DATA TEMPERED
                                Log::error('SSLCOMMERZ Validation: Data tampering detected (non-BDT)', [
                                    'merchant_tran_id' => $merchant_trans_id,
                                    'api_tran_id' => $tran_id,
                                    'merchant_amount' => $merchant_trans_amount,
                                    'api_currency_amount' => $currency_amount,
                                    'id_match' => trim($merchant_trans_id) == trim($tran_id),
                                    'amount_match' => abs($merchant_trans_amount - $currency_amount) < 1,
                                    'currency_match' => trim($merchant_trans_currency) == trim($currency_type)
                                ]);
                                $this->error = "Data has been tempered";
                                return false;
                            }
                        ...
                        # FAILED TRANSACTION
                        Log::error('SSLCOMMERZ Validation: Transaction status is not VALID', [
                            'status' => $status,
                            'tran_id' => $tran_id
                        ]);
                    ...
                    Log::error('SSLCOMMERZ Validation: Failed to connect to SSLCOMMERZ API', [
                        'http_code' => $code,
                ...
                Log::error('SSLCOMMERZ Validation: Hash verification failed', [
                    'tran_id' => $merchant_trans_id,
                    'error' => $this->error
                ]);
            ...
            # INVALID DATA
            Log::warning('SSLCOMMERZ Validation: Invalid data provided', [
