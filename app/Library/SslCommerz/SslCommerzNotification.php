            ...
            if ($code == 200 && !(curl_errno($handle))) {
                ...
                if ($status == "VALID" || $status == "VALIDATED") {
                    if ($merchant_trans_currency == "BDT") {
                        // Calculate amount difference with better tolerance (0.01 BDT minimum or 1%)
                        $amountDiff = abs($merchant_trans_amount - $amount);
                        $allowableDiff = max(0.01, $merchant_trans_amount * 0.01);

                        if (trim($merchant_trans_id) == trim($tran_id) && $amountDiff <= $allowableDiff && trim($merchant_trans_currency) == trim('BDT')) {
                            return true;
                        } else {
                            # DATA TEMPERED
                            $this->error = "Data has been tempered (ID match: " . (trim($merchant_trans_id) == trim($tran_id) ? 'YES' : 'NO') .
                                           ", Amount diff: {$amountDiff}, Allowed: {$allowableDiff}, Merchant amount: {$merchant_trans_amount}, Your amount: {$amount})";
                            return false;
                        }
                    } else {
